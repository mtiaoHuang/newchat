...
var SKIMSDK={
    ...
    initApi(url,userId,resource,token,pingTime,server){
        ...
        // SKIMSDK.server=AppConfig.boshDomain;
    },
    ...
    onmessage:function(elem){
       ...
       if(SKIMSDK.checkConflict(elem))
		...
		for (var i = 0; i < msgArr.length; i++) {
		 	...
		 	if(!message)
		 		continue;
			 //console.log("收到 receiver  "+Strophe.serialize(message));
			 //处理单条消息
			//  console.log(188)
    ...
    */
		...
		if(type ==ChatType.CHAT){//单聊
		  ...
		  if(!delay){
		  	...
		  	if(SKIMSDK.hasRequestReceipt(elem)){
		  	   SKIMSDK.sendReceipt(message.from,message.messageId);
		  	}
    ...
    buildMessage:function(msgObj){
    	...
    	var to=SKIMSDK.getUserIdFromJid(message.to);
		console.log("to "+to);
       if (SKIMSDK.isGroup(to)) {
		...
		console.log("messageReceiver  "+text.replace(/&quot;/gm, '"'));
		return elem;
    },
  	/*加入群组*/
   ...
   //检测是否为下线消息
	...
	checkReceived:function(message){
		...
		if(!received)
			return false;
		console.log("收到回执 checkReceived  "+Strophe.serialize(message));
	...
	_XEP_0045_037 : function(groupId,userId,seconds) {
		...
		console.log(to+" to _XEP_0045_037 seconds "+seconds);
		var pres = $pres({
			id : id,
			to : to
		}).c("x", {
	...
	enabledStream:function(streamId){
		...
		SKIMSDK.connection.streamManagement.addAcknowledgedStanzaListener(function(id){
			console.log("streamManagement ack  "+id);
