  ...
  methods: {
    ...
    onKeyup (e) {
      if (e.ctrlKey && e.keyCode === 13 && this.content.length) {
        this.sendGroupMessage(this.content);
        this.content = '';
      }
    }
  }
...
<template>
  ...
  <div class="message" v-scroll-bottom="groups.messages">
    <ul>
      <li v-for="item in groups.messages">
        <p class="time">
          <span>{{ item.date | time }}</span>
        </p>
        <div class="main" :class="{ self: item.self }">
          <img class="avatar" width="36" height="36" :src="item.self ? user.img : groups.user.img" />
